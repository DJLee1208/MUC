...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        for i in range(configs.embed_depth):
            self.enc_embedding.append(NASGradientSearchEmbedding(configs, first_layer = True if i == 0 else False))
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        _, _, N = x_enc.shape # x_enc.shape = (batch, seq_len, n_var)

        ############################    MUC    ############################
        '''
        x_enc가 multivariate time series input (look-back window)
